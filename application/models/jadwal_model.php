...
class Jadwal_model extends CI_Model {
	...
	public function search_auto($kelas){
		$where = "kelas = {$kelas} AND p1 != ''";
		$query = $this->db->where($where)->get('pelajaran');
	...
	public function search_auto_hari($kelas){
		$where = "";

		foreach($this->search_auto($kelas) as $row){
	...
	public function search_auto_kelas($kelas){	
